...
from PyQt6.QtWidgets import (QVBoxLayout, QHBoxLayout, QGridLayout, QFormLayout, QWidget, QFrame, QDialog, QMessageBox,
                             QListWidget, QListWidgetItem, QPushButton, QLabel, QSpacerItem, QLineEdit, QComboBox,
                             QScrollArea, QInputDialog)
import convert_functions as cf
import import_functions as tf
from mclib import UtmXmlRpc
...
class SelectAction(QWidget):
    ...
    def __init__(self, parent):
        super().__init__()
        self.parent = parent
        text1 = "<b><font color='green' size='+2'>Конвертация конфигурации CheckPoint на UG MC v.7</font></b>"
...
в текущей директории. После экспорта вы можете просмотреть результат и изменить содержимое файлов в соответствии с вашими потребностями."
        text3 = "Импорт файлов конфигурации из каталога 'data_ug' в шаблон NGFW области UserGate Management Center версии 7."
        label1 = QLabel(text1)
        ...
        form = QFormLayout()
        form.addRow(QLabel("<font color='blue'>Экспорт конфигурации:</font>"), label2)
        form.addRow(QLabel("<font color='blue'>Импорт конфигурации:</font>"), label3)
...
class SelectDestinationTemplate(QWidget):
    """Класс для выбора шаблона для импорта. Номер в стеке 2."""
    def __init__(self, parent):
        super().__init__()
        self.parent = parent

        title = QLabel("<b><font color='green' size='+2'>Выбор шаблона для импорта</font></b>")
        title.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        label = QLabel("Выберите шаблон для импорта конфигурации или создайте новый.")
        label.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        ...
        self.btn2.clicked.connect(self.run_next_page)
        ...
        self.parent.stacklayout.currentChanged.connect(self.add_device_template_items)
    ...
    def enable_button3(self):
        self.btn3.setStyleSheet('color: sienna; background: white;')
        self.btn3.setEnabled(True)

    def add_device_template_items(self, e):
        """
        При открытии этой вкладки получаем с МС список шаблонов устройств и заполняем список выбора шаблонов.
        """
        if e == 2:
            if not self.get_auth():
                ...
                if err:
                    ...
                    for item in result:
                        ...
                        self.parent.dest_templates[item['name']] = item['id']
    ...
    def get_auth(self):
        ...
        if not self.parent.utm:
            dialog = LoginWindow(parent=self)
            result = dialog.exec()
            if result == QDialog.DialogCode.Accepted:
                self.parent.utm = dialog.utm
                if self.parent.utm.version_hight < 7:
                    message_inform(self, "Внимание!", "Импорт возможен только на МС версии 7 и выше!")
                    return False
                return True
            else:
                return False
        return self.parent.check_auth()
    ...
    def create_new_template(self):
        ...
        if ok:
            if self.parent.check_auth():
                err, result = self.parent.utm.add_device_template({'name': template_name, 'description': 'Шаблон для импорта с Check Point'})
                if err:
                    message_alert(self, result, 'Не удалось добавить шаблон устройства!')
                else:
                    message_inform(self, 'Создание шаблона', f'Создан шаблон "{template_name}".')
                    self.add_device_template_items(2)
            else:
                self.run_page_0()

    def select_dest_template(self, item_text):
        self.parent.current_dest_template = item_text
        self.enable_button2()
    ...
    def run_page_0(self):
        """При переходе на первую вкладку, очищаем список шаблонов."""
        if self.parent.utm:
            self.parent.utm.logout()
        self.device_templates_list.clear()
        self.parent.utm = None
        self.parent.dest_templates.clear()
        self.parent.current_dest_template = None
        self.parent.stacklayout.setCurrentIndex(0)
...
class SelectImportMode(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.btn1.clicked.connect(self.run_page_2)
...
#        self.btn3.setEnabled(True)
    ...
    def select_item(self, item_text):
        ...
        self.template_id = self.parent.dest_templates[self.parent.current_dest_template]
...
#        return
                ...
                if self.thread is None:
                    ...
                    if point_number in ('6', '7', '8'):
                        ...
                        if point_number ==  '6':
                            ...
                            self.thread = self.func[self.selected_point](self.parent.utm, self.template_id, node_name)
                    else:
                        self.thread = self.func[self.selected_point](self.parent.utm, self.template_id)
    ...
    def import_all(self):
        ...
        if result == 'yes':
            self.disable_buttons()
            cv = CreateVlans(self, self.parent.utm)
            if self.thread is None:
                self.thread = tf.ImportAll(self.parent.utm, cv.utm_vlans, cv.utm_zones, cv.new_vlans, cv.ifaces)
    ...
    def run_page_2(self):
        """Переходим на предыдущую вкладку, очищаем список и лог."""
        self.sg_list.clear()
        self.log_list.clear()
        self.parent.stacklayout.setCurrentIndex(2)
...
class CreateVlans:
    ...
    def __init__(self, parent, utm, template_id):
        self.utm = utm
        self.parent = parent
        ...
        _, result = self.utm.get_template_zones_list(self.template_id)
        ...
        vlans = sorted([item['vlan_id'] for item in self.ifaces if item['kind'] == 'vlan'])
