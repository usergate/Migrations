...
def convert_config_file(parent, path):
    ...
    if parent.error:
        error = 1
    out_message = f'GREEN|    Конфигурация Fortigate в формате json выгружена в файл "{json_file}".'
...
def convert_dns_servers(parent, path, data):
    ...
    if 'config user domain-controller' in data:
        ...
        parent.stepChanged.emit(f'GREEN|    Правила DNS в DNS-прокси выгружены в файл "{json_file}".')
    else:
...
def convert_services(parent, path, data):
    ...
    out_message = f'GREEN|    Сервисы выгружены в файл "{json_file}".'
...
def convert_zone_settings(parent, path, data):
    ...
    if 'config system zone' in data:
        ...
        for key, value in data.get('config system zone', {}).items():
            zone_name = func.get_restricted_name(key.strip())
...
def convert_ip_lists(parent, path, data):
    ...
    for list_name, value in data.get('config firewall multicast-address', {}).items():
        ip_list = {
            'name': 'Multicast - ' + func.get_restricted_name(list_name.strip()),
            'description': '',
            'type': 'network',
            'url': '',
            'list_type_update': 'static',
        ...
        if value and 'type' not in value:
            if 'mappedip' in value:
                ...
                if value['mappedip'] not in ips_for_rules:
                    ips_for_rules.add(func.create_ip_list(parent, path, ips=[value['mappedip']], name=key))
                if 'service' in value:
            ...
            for item in value['member'].split(','):
                ...
                if item in parent.ip_lists:
                    ...
                    try:
                        ...
                        pass
            if ip_list['content']:
                ip_list['name'] = func.get_restricted_name(list_name.strip())
...
def convert_firewall_policy(parent, path, data):
    ...
    for key, value in data['config firewall policy'].items():
        ...
        rule = {
            'name': func.get_restricted_name(rule_name.strip()),
    ...
    if 'router-id' in bgp:
        ...
        try:
            ...
            parent.vrf['bgp'] = {
                ...
                'multiple_path': False,
                'redistribute': ['connected'] if bgp['config redistribute connected'].get('status', None) == 'enable' else [],
