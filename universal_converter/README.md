...
4. <b>Для Fortigate:</b><br>
4.1. Переименуйте файл конфигурации Fortigate в fortigate.cfg и скопируйте его в каталог data_fortigate/<i><имя_которое_вы_задали_на_предыдущем_шаге></i><br>
5. <b>Для Huawei:</b><br>
5.1. Переименуйте файл конфигурации Huawei в huawei.cfg и скопируйте его в каталог data_huawei/<i><имя_которое_вы_задали_на_предыдущем_шаге></i><br>
...
8. <b>Импорт конфигурации:</b><br>
8.1. Перед началом работы на новой версии UserGate NGFW настроить DNS, интерфейс Untrusted (для выхода в интернет),
шлюз, произвести активацию корректным ПИН-кодом. После этого дождаться обновления библиотек и списков.<br>
8.2. Запустить программу <b>universal_converter</b>.<br>
8.3. Выбрать пункт 2 - <b>Импорт конфигурации</b>.<br>
8.4. Программа попросит выбрать каталог для импорта конфигурации, затем появиться окно авторизации.
...
8.5. Можно импортировать всю конфигурацию, только выбранную группу разделов или отдельный раздел.<br>
8.6. Если у вас используются собственные сертификаты, то перед импортом необходимо импортировать их или создать с теми же именами.<br>
8.7. Если у Вас используются сервера аутентификации (LDAP, NTLM, RADIUS, TACACS, SAML), то необходимо в первую очередь импортировать
...
8.8. Импорт некоторых разделов конфигурации может идти долго. Не надо думать что "программа зависла". Надо дождаться окончания импорта.<br>
8.9. После импорта проверьте вывод программы на наличие сообщений, выделенных цветом. Исправьте конфигурацию NGFW в соответствие
с рекомендациями.<br>
9. Можно сохранить лог импорта в файл import.log. Файл создаётся в выбранном каталоге импорта конфигурации.<br>
...
6. <b>Экспорт с Huawei:</b><br>
6.1. При экспорте правил security-policy, geo-location с обозначениями стран не переносятся. После импорта на UG NGFW в соответствующих
...
6.2. При экспорте правил security-policy, profile не конвертируются.<br>
6.3. При экспорте правил traffic-policy, указанные интерфейсы не конвертируются. После импорта на UG NGFW в соответствующих правилах
пропускной способности необходимо указать зоны, соответствующие нужным интерфейсам.<br>
6.4. Правила policy-based-route не переносятся, так как в UG NGFW используется другая идеология для построения правил ПБР. В общем
...
6.5. Правила МЭ и NAT/DNAT после импорта находятся в неактивном состоянии. Проверьте их и внесите необходимые изменения. На UserGate NGFW
...
7.5. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
...
8.3. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
9. <b>Импорт на версию UG NGFW 7.0:</b><br>
9.1. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
10. <b>Импорт на версию UG NGFW 7.1:</b><br>
