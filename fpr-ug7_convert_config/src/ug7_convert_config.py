...
#
#--------------------------------------------------------------------------------------------------- 
# Программа предназначена для переноса конфигурации с устройств Cisco FPR-2130 на NGFW UserGate версии 7.
# Версия 1.0
#
...
from services import character_map, network_proto, service_ports, character_fwname
from ug7_utm import UTM
...
def convert_file(file_name):
    ...
    trans_fwname = str.maketrans(character_fwname)
    data = {
        'ip_lists': {},
        'services': {},
        'ifaces': [],
    ...
    def convert_access_list(rule_id, rule_name, fh):
        fw_rule = {
            'name': rule_name.translate(trans_fwname).strip("-").strip(),
            'description': "",
            'action': '',
            'src_zones': set(),
            'dst_zones': set(),
...
def import_firewall_rules(utm, fw_rules):
    ...
    for item in fw_rules.values():
        ...
        try:
            item['services'] = [['service', services_list[x]] for x in item['services']]
        except KeyError as err:
...
def import_routes(utm, static_routes):
    ...
    err, result = utm.update_routers(virt_routers['default'], vrf_info)
    if err == 2:
        print(f'\033[31m{result}\033[0m')
    else:
...
#        sys.exit(0)
        ...
        try:
            ...
            if int(utm.version[:1]) != 7:
                print("\033[31mВы подключились к NGFW UG версии ниже 7. \nДанная программа предназначена для импорта конфигурации только на NGFW версии 7.\033[0m")
                utm.logout()
                sys.exit(1)
