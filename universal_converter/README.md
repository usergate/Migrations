...
поведение будет исправлено в версии МС 7.1.3.<br>
...
котором объект присутствует. Шаблоны других групп не проверяются.
3. Если у Вас используются сервера аутентификации (LDAP, NTLM, RADIUS, TACACS, SAML), то необходимо настроить LDAP-коннектор
...
4. Перед импортом убедитесь, что в шаблоне Management Center существуют интерфейсы и зоны. Они необходимы для создания
...
зоны, затем создать необходимые интерфейсы для будущих VLAN, шлюзов и VRF и только затем импортировать всё.<br>
5. После импорта раздела UserGate/Настройки включите синхронизацию необходимых подразделов.<br>
6. Если при импорте правила в шаблон МС произошли ошибки, то данное правило становится не активным и в описание правила
...
7. Правила МЭ, КФ и т.д. добавляются в конец списка правил. Это надо учитывать если вы повторно импортируете конфигурацию в
шаблон после изменений в файлах json. Не меняйте имена правил, иначе правила продублируются в конец списка правил.<br>
8. <b>Не следует импортировать конфигурацию, полученную с UserGate NGFW.</b> В этом конвертере импорт всех разделов не
