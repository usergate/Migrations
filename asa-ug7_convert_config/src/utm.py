...
################################### Settings ####################################
    ...
    def add_certificate(self, cert):
        ...
        try:
            ...
            return 1, f"Error utm.add_certificate: [{err.faultCode}] — {err.faultString}: {admin['login']}"
        else:
            return 0, result     # Возвращает ID добавленного правила
...
######################################## Маршруты  ########################################
    ...
    def get_ospf_config(self):
        ...
        try:
            ...
            return 1, f"Error utm.get_ospf_config: [{err.faultCode}] — {err.faultString}"
    ...
    def get_bgp_config(self):
        ...
        try:
            ...
            return 1, f"Error utm.get_bgp_config: [{err.faultCode}] — {err.faultString}"
...
################################### Пользователи и устройства #####################################
    ...
    def add_group(self, group):
        ...
        try:
            ...
            if err.faultCode == 409:
                ...
                return 1, f"Недопустимые символы в названии группы: '{group['name']}'! Возможно используются русские буквы."
            else:
    ...
    def update_user(self, guid, user):
...
#        guid = user['id'] if (self.version_hight >= 7 and self.version_midle >= 1) else user['guid']
    ...
    def get_auth_servers(self):
        ...
        try:
            ...
            return 1, f"Error utm.get_auth_servers: [{err.faultCode}] — {err.faultString}"
...
#            return 1, f"Сервер авторизации '{server['name']}' уже существует."
        try:
            ...
            if err.faultCode == 111:
                return 1, f"Недопустимые символы в названии auth-сервера! Возможно используются русские буквы."
            else:
...
#            self.auth_servers[server['name']] = result
    ...
    def add_auth_profile(self, profile):
        ...
        if profile['name'] in self.auth_profiles.keys():
            return 2, f"Профиль авторизации '{profile['name']}' уже существует."
        try:
            ...
            if err.faultCode == 110:
                ...
                return 1, f"Недопустимые символы в названии auth-профиля '{profile['name']}'. Возможно используются русские буквы."
            else:
                return 1, f"\tОшибка utm.add_auth_profile: [{err.faultCode}] — {err.faultString}"
        else:
    ...
    def add_captive_profile(self, profile):
        ...
        try:
            ...
            if err.faultCode == 110:
                ...
                return 1, f"Недопустимые символы в названии captive-профиля '{profile['name']}'. Возможно используются русские буквы."
            else:
    ...
    def add_captive_portal_rules(self, rule):
        ...
        if rule['name'] in self.captive_portal_rules.keys():
            return 2, f'Правило Captive-портала "{rule["name"]}" уже существует.'
        try:
            ...
            if err.faultCode == 110:
                ...
                return 1, f"Недопустимые символы в названии правила captive-портала '{rule['name']}'. Возможно используются русские буквы."
            else:
    ...
    def add_byod_policy(self, rule):
        ...
        if rule['name'] in self.byod_rules.keys():
            return 2, f'Правило BYOD "{rule["name"]}" уже существует.'
        try:
    ...
    def get_ldap_user_name(self, user_guid):
        ...
        try:
            ...
            if err.faultCode == 1:
                return 2, f'Не возможно получить имя доменного пользователя. Проверьте что версия UTM 5.0.6.4973 (6.1.3.10697) или выше.'
            else:
    ...
    def get_ldap_group_name(self, group_guid):
        ...
        try:
            ...
            if err.faultCode == 1:
                return 2, f'Не возможно получить имя доменной группы. Проверьте что версия UTM 5.0.6.4973 (6.1.3.10697) или выше.'
            else:
