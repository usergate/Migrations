...
получим ошибку.<br>
3. <b>Экспорт с Fortigate:</b><br>
3.1. При экспорте сервисов, поля "iprange" и "fqdn" не переносятся так как не поддерживаются в UG NGFW.<br>
3.2. При экспорте групп пользователей, доменные пользователи не переносятся так как не известно к какому домену они относятся.<br>
3.3. Правила NAT не переносятся так как идеология натирования в UG NGFW очень сильно отличается от Fortigate.<br>

4. <b>Импорт на версию UG NGFW 5:</b><br>
4.1. На версию 5 не импортируются VLAN-ы, Маршруты, OSPF, BGP.<br>
4.2. При импорте не переносятся группы сервисов и IP-адресов, так как в версии 5 группы не поддерживаются.<br>
4.3. На версию 5 не возможно импортировать содержимое календарей. Содержимое надо добавить руками.<br>
4.4. На версию 5 не возможно импортировать содержимое URL категорий. Содержимое надо добавить руками.<br>
4.5. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
5. <b>Импорт на версию UG NGFW 6:</b><br>
5.1. Конвертер не работает с UG NGFW версии 6 если она старее 6.1.7. В этом случае обновитесь до последней версии 6.<br>
5.2. При импорте не переносятся группы сервисов и IP-адресов, так как в версии 6 группы не поддерживаются.<br>
5.3. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
6. <b>Импорт на версию UG NGFW 7.0:</b><br>
6.1. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
7. <b>Импорт на версию UG NGFW 7.1:</b><br>
