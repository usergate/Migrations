...
class ImportSelectedPoints(QThread, ReadWriteBinFile, MyMixedService):
    """Импортируем разделы конфигурации в шаблон MC"""
...
#                item.pop('master', None)
    ...
    def import_ipip_interfaces(self, path, data):
        ...
        mc_ifaces = self.mc_data['interfaces']
#        mc_gre = [int(x[3:].split(':')[0]) for x in mc_ifaces if x.startswith('gre') and x.split(':')[1] == self.node_name]
        ...
        for item in self.iface_settings:
            if 'kind' in item and item['kind'] == 'vlan':
                ...
                if current_zone == "Undefined":
                    ...
                    try:
                        item['zone_id'] = self.mc_data['zones'][current_zone].id
                    except KeyError as err:
                        self.stepChanged.emit(f'RED|       Error: [Интерфейс "{item["name"]}"] Не найдена зона {err}. Импортируйте зоны и повторите попытку.')
                        item['zone_id'] = 0
                        error = 1
    ...
    def import_dhcp_subnets(self, path):
        ...
        if isinstance(self.ngfw_ports, list) and not self.dhcp_settings:
            ...
            err, self.dhcp_settings = self.read_json_file(json_file, mode=1)
            if err:
                return
            if not self.mc_data['interfaces']:
                if self.get_interfaces_list():        # Получаем все интерфейсы группы шаблонов и заполняем: self.mc_data['interfaces']
        ...
        err, result = self.utm.get_template_dhcp_list(self.template_id)
        if err:
        ...
        for item in self.dhcp_settings:
            ...
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя DHCP')
            if item['name'] in mc_dhcp_subnets:
                self.stepChanged.emit(f'uGRAY|    DHCP subnet "{item["name"]}" уже существует в текущем шаблоне на узле кластера "{self.node_name}".')
                continue
    ...
    def import_vrf(self, path):
        ...
        for item in data:
            ...
            for x in item['routes']:
                error, x['name'] = self.get_transformed_name(x['name'], err=error, descr='Имя статического маршрута')
        ...
        for uid, name in self.templates.items():
            ...
            for x in result:
                if x['name'] in wccp_rules:
                    self.stepChanged.emit(f'RED|    Правило WCCP "{x["name"]}" обнаружено в нескольких шаблонах группы шаблонов. Правило из шаблона "{name}" не будет использовано.')
                else:
                ...
                for x in item['routers']:
                    if x[0] == 'list_id':
                        try:
                            ...
                            self.stepChanged.emit(f'ORANGE|    Не найден список {err} для правила "{item["name"]}" в группе шаблонов. Загрузите списки IP-адресов и повторите попытку.')
                            continue
                    routers.append(x)
            ...
            if 'cert.pem' in files:
                ...
                if data['name'] in mc_certs:
                    ...
                    if err == 1:
                        ...
                        self.stepChanged.emit(f'LBLUE|       Необходимо назначить роль новому сертификату "{cert_name}".')
                    continue
...
#            self.msleep(10)
            ...
            if key in params:
                    ...
                    if isinstance(value, dict):
                        if value['type'] == 'pki':
                            try:
                                ...
                                self.stepChanged.emit(f'RED|    Не найден профиль клиентского сертификата {err} для "{params[key]}". Загрузите профили клиентских сертификатов и повторите попытку.')
                                error = 1
                                continue
                if key == 'web_console_ssl_profile_id':
                    try:
                        ...
                        self.stepChanged.emit(f'RED|    Не найден профиль SSL {err} для "{params[key]}". Загрузите профили SSL и повторите попытку.')
                        error = 1
                        continue
                ...
                if key == 'endpoint_certificate_id':
                    try:
                        ...
                        self.stepChanged.emit(f'RED|    Не найден сертификат {err} для "{params[key]}". Загрузите сертификаты и повторите попытку.')
                        error = 1
                        continue
                setting = {}
            ...
            if key in params:
                ...
                if key == 'tunnel_inspection_zone_config':
                    try:
                        ...
                        self.stepChanged.emit(f'RED|    Не найдена зона {err} для "{params[key]}". Загрузите зоны и повторите попытку.')
                        error = 1
                        continue
                setting = {}
                setting[key] = {'value': value}
    ...
    def import_proxy_exceptions(self, path):
        ...
        if exceptions:
            ...
            if err:
                self.stepChanged.emit(f'RED|    {result}')
                error = 1
                self.stepChanged.emit('ORANGE|    Произошла ошибка при установке статуса исключения кэширования.')
            else:
    ...
    def import_web_portal_settings(self, path):
        ...
        try:
            ...
            self.stepChanged.emit('RED|{message}\n    Произошла ошибка при импорте настроек Веб-портала.')
            self.error = 1
            return
        ...
        if err:
            ...
            self.stepChanged.emit('GREEN|    Импорт настроек Веб-портала завершён.')
    ...
    def import_upstream_proxy_settings(self, path):
        ...
        if err:
            ...
            self.stepChanged.emit('GREEN|    Настройки вышестоящего прокси импортированы.')
    ...
    def import_upstream_update_proxy_settings(self, path):
        ...
        if err:
            ...
            self.stepChanged.emit('GREEN|    Настройки вышестоящего прокси для проверки лицензий и обновлений импортированы.')
    ...
    def import_local_groups(self, path):
        ...
        for item in groups:
            ...
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя группы пользователей')
            ...
            for user_name in users:
                ...
                if len(user_array) > 1 and ('\\' in user_array[1]):
                    ...
                    try:
                        ...
                        self.stepChanged.emit(f'bRED|       Доменный пользователь "{user_name}" не импортирован в группу "{item["name"]}". Нет LDAP-коннектора для домена "{domain}".')
                    else:
                        ...
                        if err1:
                            ...
                            self.stepChanged.emit(f'bRED|       Нет пользователя "{user_name}" в домене "{domain}". Доменный пользователь не импортирован в группу "{item["name"]}".')
                            continue
    ...
    def import_ldap_servers(self, path, ldap_servers):
        ...
        for item in data:
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя LDAP-коннектора')
    ...
    def import_ntlm_server(self, path, ntlm_servers):
        ...
        for item in data:
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя сервера NTLM')
    ...
    def import_radius_server(self, path, radius_servers):
        ...
        self.stepChanged.emit(f'LBLUE|       После импорта необходимо в каждом сервере RADIUS ввести пароль.')
        error = 0

        for item in data:
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя сервера RADIUS')
    ...
    def import_tacacs_server(self, path, tacacs_servers):
        ...
        self.stepChanged.emit(f'LBLUE|       После импорта необходимо в каждом сервере TACACS+ ввести секретный ключ.')
        error = 0

        for item in data:
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя сервера TACACS')
    ...
    def import_saml_server(self, path, saml_servers):
        ...
        self.stepChanged.emit(f'LBLUE|       После импорта необходимо в каждый сервер SAML загрузить SAML metadata.')
        error = 0

        for item in data:
            error, item['name'] = self.get_transformed_name(item['name'], err=error, descr='Имя сервера SAML')
