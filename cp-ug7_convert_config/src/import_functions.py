...
def import_url_lists(parent):
    ...
    for file_name in files_list:
        ...
        if err == 1:
            parent.stepChanged.emit(f'1|{result}')
            parent.stepChanged.emit(f'2|Ошибка! Список URL "{data["name"]}" не импортирован.')
            error = 1
            continue
        elif err == 2:
            parent.stepChanged.emit(f'2|{result}')
...
def import_url_categories(parent):
    ...
    err, data = read_json_file(json_file, '1|Ошибка импорта групп URL категорий!', '1|Нет групп URL категорий для импорта.')
    if err:
        parent.stepChanged.emit(data)
        parent.error = 1
        return
    ...
    for item in data:
        ...
        if item['name'] not in ['Parental Control', 'Productivity', 'Safe categories', 'Threats',
            ...
            if err == 1:
                parent.stepChanged.emit(f'1|{result}')
                parent.stepChanged.emit(f'2|Ошибка! Группа URL категорий "{item["name"]}" не импортирована.')
                error = 1
                continue
            elif err == 2:
                parent.stepChanged.emit(f"2|{result}")
            ...
            for category in content:
                try:
                    ...
                    parent.stepChanged.emit(f'2|   Ошибка! URL категория "{category["name"]}" не импортирована. Нет такой категории на UG NGFW.')
                    error = 1
                    continue
                ...
                if err2 == 1:
                    ...
                    parent.stepChanged.emit(f'2|   Категория URL "{category["name"]}" уже существует.')
                else:
                    parent.stepChanged.emit(f'2|   Добавлена категория "{category["name"]}".')

    if error:
        parent.error = 1
    out_message = '1|Группы URL категорий импортированы в раздел "Библиотеки/Категории URL".'
    parent.stepChanged.emit('1|Произошла ошибка при импорте групп URL категорий!' if error else out_message)
...
def import_application_groups(parent):
    ...
    err, data = read_json_file(json_file, '1|Ошибка импорта групп приложений!', '1|Нет групп приложений для импорта.')
    if err:
        parent.stepChanged.emit(data)
        parent.error = 1
        return
    ...
    for item in data:
        ...
        if err == 1:
            parent.stepChanged.emit(f'1|{result}')
            parent.stepChanged.emit(f'2|Ошибка! Группа приложений "{item["name"]}" не импортирована.')
            error = 1
            continue
        elif err == 2:
        ...
        for app in content:
            ...
            try:
                ...
                parent.stepChanged.emit(f'2|   Ошибка! Приложение "{app_name}" не импортировано. Такого приложения нет на UG NGFW.')
                error = 1
                continue
    ...
    if error:
        parent.error = 1
    out_message = '1|Группы приложений импортированы в раздел "Библиотеки/Приложения".'
    parent.stepChanged.emit('1|Произошла ошибка при импорте групп приложений!' if error else out_message)
...
def import_firewall_rules(parent):
    ...
    err, data = read_json_file(json_file, '1|Ошибка импорта правил межсетевого экрана!', '1|Нет правил межсетевого экрана для импорта.')
    if err:
        parent.stepChanged.emit(data)
        parent.error = 1
        return
    ...
    for item in data:
        ...
        for service in item['services']:
            try:
                ...
                error = 1
                parent.stepChanged.emit(f'3|Error! Не найден сервис {service} для правила {item["name"]}.')
        for app in item['apps']:
            try:
                app[1] = l7categories[app[1]] if app[0] == 'ro_group' else applicationgroup[app[1]]
            except KeyError as err:
                error = 1
                parent.stepChanged.emit(f'3|Error! Не найдена группа приложений {err} для правила "{item["name"]}". Загрузите группы приложений и повторите попытку.')
            ...
            if x[0] == 'user' and x[1]:
                ...
                if i[2]:
                    ...
                    if err:
                        ...
                        parent.stepChanged.emit(f'2|   Ошибка! Нет LDAP-коннектора для домена "{i[0]}"! Импортируйте и настройте LDAP-коннектор. Затем повторите импорт.')
                    else:
                        x[1] = result
                        users.append(x)
