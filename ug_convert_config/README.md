...
Программа предназначена для переноса конфигурации с NGFW версий 5, 6 и 7.0 на NGFW версий 6 и 7.0
...
Для работы программы на интерфейсе Management в версии 5 необходимо включить сервис xml-rpc. На версиях 6 и 7 этого
...
1. Экспорт конфигурации с версий 5, 6, 7:<br>
...
1.3. Запустить <b>ug_convert_config</b> в терминале.<br>
...
администратора NGFW.<br>
1.5. Выбрать пункт 1 - <b>Экспорт конфигурации</b>. В каталоге data будут созданы файлы json с текущей конфигурацией.<br>
1.6. После экспорта проверьте вывод программы для исправления найденных ошибок. Исправьте текущую конфигурацию UTM и повторите экспорт.
2. Импорт конфигурации на версии 6 и 7:<br>
...
логин которого используете, в профиле включены разрешения для API. Указать ip и login/пароль администратора NGFW.<br>
2.4. Выбрать пункт 2 - <b>Импорт конфигурации</b>.<br>
2.5. Если у Вас используются сервера аутентификации (LDAP, NTLM, RADIUS, TACACS, SAML), то необходимо в первую очередь импортировать
...
загрузить keytab-файл, если используется авторизация kerberos и проверить работу данных серверов аутентификации. Затем можно выбрать
<b>"Импортировать всё"</b>.<br>
Конфигурация из каталога data будет импортирована на NGFW.<br>
2.6. После импорта проверьте вывод программы на наличие сообщений, выделенных цветом. Исправьте конфигурацию UTM в соответствие с рекомендациями.

<b>Примечание:</b> Если вы используете UG UTM версии старее 6.1.7, то используйте архив ug_convert_config_old.zip, только при его использовании
нельзя сделать импорт на версию 7.

<b>Важно:</b>
1. Если вы делаете экспорт из версии 5, то выгрузка из правил доменных пользователей и доменных групп для дальнейшего переноса возможна
только при использовании версии UTM равной 5.0.6.4973. Если у вас версия 4825, поставьте update до версии 4973, который можно запросить
в техподдержке UG.<br>
<b>Не используйте версию 4973 для работы в боевом режиме. Она предназначена только для экспорта конфигурации и последующего немедленного
перехода на версию 6.</b><br>
Импорт конфигурации поддерживается на последние текущие версии 6 и 7.
...
4. Имена групп, серверов аутентификации, различных профилей, правил, всех списков библиотеки и всего-всего должны быть ТОЛЬКО в английском регистре.
Русские буквы допустимы только в описании. Перед экспортом, замените русские символы в именах на английские.
5. После импорта профилей администраторов, необходимо проверить доступ к разделам, так как в версии 6 и 7 были добавлены
новые разделы. При импорте доступ к разделам не меняется и, следовательно, доступ к новым разделам не предоставляется.
Профили с русскими именами не переносятся в версию 7 (в ней запрещены русские имена). Перед экспортом, замените имена профилей на английские.
6. Если существуют правила МЭ с одинаковыми именами, то при конвертации к имени такого правило добавляется номер,
что бы имена были уникальными.
7. Пробелы в начале и конце имён правил, списков и т.д. при конвертации удаляются.
8. Имена списков IP-адресов типа "10.10.10.0/24" заменяются на "10.10.10.0_24" так как при переносе каждый список
записывается в отдельный файл с данным именем, а в файловой системе Linux прямой слэш является разделителем пути к файлу.
9. После импорта настроек BGP в свойсвах каждого из bgp-соседей надо заново ввести пароль.
10. Никакие пароли (локальных пользователей, PPPoE, VPN, серверов аутентификации и т.д.) не переносятся. Необходимо заново вручную ввести пароль.
Это ограничение API - невозможно выгрузить парольную информацию.
11. При импорте интерфейсов им прописываются IP-адреса из сохранённой конфигурации. Необходимо проследить, чтобы не было
...
12. Раздел Библиотеки "Профили СОВ": не импортируются сигнатуры профилей СОВ с версии 5 на версию 6, так как
в 6-ой версии структура и состав сигнатур кардинально поменялись.
13. Сертификаты не переносятся. Если используется сертификат по умолчанию для SSL инспектирования (CA Default), то необходимо
...
14. Правила экспорта настроек на сервера FTP, SSH не переносятся. Это надо сделать руками.
15. Если у вас что то не получается или есть предложения и замечания, пожалуйста, откройте тикет в тех.поддержку компании UserGate.

20.11.2023  Исправлены ошибки экспорта локальных пользователей и групп, профилей DoS.<br>
03.11.2023  Сделана обработка ошибки получения SNMP Engine ID для версии 5, где этого параметра нет.<br>
23.08.2023  Сделан импорт списков IP-адресов, содержащих другие списки IP-адресов.<br>
10.08.2023  Исправлен экспорт списков библиотеки, содержащих большое количесво записей.<br>
09.08.2023  Исправлен импорт локальных групп содержащих локальных пользователей.<br>
08.06.2023  Исправлен экспорт календарей.<br>
22.05.2023  Добавлена совместимость с версией NGFW 7.<br>
13.02.2023  Исправлена совместимость экспорта списка исключений кеширования HTTP для версий старше 6.1.7.<br>
29.11.2022  Исправлена ошибка импорта локальных пользователей.<br>
25.08.2022  Исправлены ошибки экспорта/импорта листов в версии 6.1.8.11532R.<br>
