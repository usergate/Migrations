...
3. При импорте правил SNMP, правила v.3 импортируются как v.2<br>
4. После импорта проверьте вывод программы на наличие сообщений, выделенных цветом. Исправьте конфигурацию в соответствие
с рекомендациями.<br>
5. Можно сохранить лог импорта в файл import.log. Файл создаётся в выбранном каталоге импорта конфигурации.<br>
...
3. Выбрать пункт 2 - <b>Импорт конфигурации</b>.<br>
...
5. Если у Вас используются сервера аутентификации (LDAP, NTLM, RADIUS, TACACS, SAML), то необходимо в первую очередь импортировать
...
6. Импорт некоторых разделов конфигурации может идти долго. Не надо думать что "программа зависла". Надо дождаться окончания импорта.<br>
7. <b>Импорт на версию UG NGFW 5:</b><br>
7.1. На версию 5 не импортируются VLAN-ы, Маршруты, OSPF, BGP.<br>
7.2. При импорте не переносятся группы сервисов и IP-адресов, так как в версии 5 группы не поддерживаются.<br>
7.3. На версию 5 не возможно импортировать содержимое календарей. Содержимое надо добавить руками.<br>
7.4. На версию 5 не возможно импортировать содержимое URL категорий. Содержимое надо добавить руками.<br>
8. <b>Импорт на версию UG NGFW 6:</b><br>
8.1. Конвертер не работает с UG NGFW версии 6 если она старее 6.1.7. В этом случае обновитесь до последней версии 6.<br>
8.2. При импорте не переносятся группы сервисов и IP-адресов, так как в версии 6 группы не поддерживаются.<br>

<b>Импорт конфигурации в шаблон Management Center:</b>
1. В настоящее время при импорте объектов в библиотеки на МС версии 7 не учитывается регистр символов. То есть одинаковые имена
в разных регистрах считаются одинаковыми! МС выдаёт сообщение, что такое имя уже существует и не импортирует объект. Данное
поведение будет исправлено в версии МС 7.1.2.<br>
2. Если у Вас используются сервера аутентификации (LDAP, NTLM, RADIUS, TACACS, SAML), то необходимо настроить LDAP-коннектор
в разделе "Управление областью/Каталоги пользователей" и проверить его работу. Данный LDAP-коннектор будет использоваться для
поиска доменных пользователей и групп в правилах.<br>
3. Перед импортом убедитесь, что в шаблоне Management Center существуют интерфейсы и зоны. Они необходимы для создания
VLAN, subnet DHCP, Gateways и VRF. Если нет интерфейсов то VLAN, subnet DHCP, Gateways и VRF не будут созданы. Рекомендуется в
первую очередь импортировать зоны, затем создать необходимые интерфейсы для будущих VLAN, шлюзов и VRF и только затем можно
импортировать всё.<br>
4. Если при импорте правила в шаблон МС произошли ошибки, то данное правило становится не активным и в описание правила
добавляются сообщения о произошедших ошибках. После импорта проверьте все неактивные правила на предмет ошибок конвертации.<br>
5. Правила МЭ, КФ и т.д. добавляются в конец списка правил. Это надо учитывать если вы повторно импортируете конфигурацию в
шаблон после изменений в файлах json. Не меняйте имена правил, иначе правила продублируются в конец списка правил.<br>
6. <b>Не следует импортировать конфигурацию, полученную с UserGate NGFW.</b> В этом конвертере импорт всех разделов не
реализован. Кроме того импорт реализованных разделов может привести к проблемам, так как не все поля объектов будут перенесены.

<b>Ограничения:</b>
1. Импортируются только интерфейсы VLAN и Tunnel. Все остальные интерфейсы надо будет настроить руками.<br>
2. На всех версиях (кроме 5 и 7.1) нельзя добавить более 8 полос пропускания в библиотеку. При добавлении большего количества
получим ошибку.<br>
...
<b>Errors:</b><br>
На ПК с видеокартами NVIDIA возможна ошибка: "libEGL warning: egl: failed to create dri2 screen"<br>
...
23.05.2024  Сделан экспорт статических записеё DNS и исправлены ошибки экспорта сервисов для Cisco ASA.<br>
21.05.2024  Исправлен импорт правил в шаблон МС.<br>
16.05.2024  Доработан экспорт зон для Fortigate.<br>
14.05.2024  Измененён импорт правил в шаблон МС.<br>
05.05.2024  Исправлена ошибка определения версии специализированных сборок.<br>
05.05.2024  Исправлен экспорт правил МЭ и календарей для Fortigate.<br>
