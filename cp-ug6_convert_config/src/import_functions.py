...
#
#-------------------------------------------------------------------------------------------------------- 
# Классы импорта разделов конфигурации CheckPoint на NGFW UserGate версии 6.
# Версия 2.7
#
...
def import_services_groups(parent):
    ...
    out_message = '5|Группы сервисов импортированы в раздел "Библиотеки/Сервисы".'
    error = 0
    ...
    if os.path.isdir('data_ug/Libraries/ServicesGroups'):
        ...
        if files_list:
            for file_name in files_list:
                ...
                err, data = read_json_file(json_file, '2|Ошибка импорта группы сервисов!', '2|Нет группы сервисов для импорта.')
                if err:
                    parent.stepChanged.emit(services_group)
                    parent.error = 1
                    return

                if data['name'] in services_list:
                    parent.stepChanged.emit(f'2|Сервис "{data["name"]}" уже существует.')
                else:
                    ...
                    if content:
                        service = {
                            ...
                            'protocols': [item['protocols'][0] for item in content]
                        }
                        err, result = parent.utm.add_service(service)
                        if err:
                            parent.stepChanged.emit(f'{err}|{result}')
                            if err == 1:
                                error = 1
                            ...
                            services_list[data['name']] = result
                            parent.stepChanged.emit(f'2|Сервис "{data["name"]}" добавлен.')
                    else:
...
def import_ip_lists(parent):
    ...
    for file_name in files_list:
        ...
        if content:
            ...
            for item in content:
                if 'list' in item:
                    data_file = f"{item['list'][1]}.json"
                    with open(f"data_ug/Libraries/IPAddresses/{data_file}", "r") as fh:
                        data = json.load(fh)
                    tmp_content.extend(data['content'])
                else:
            ...
            try:
                ...
                error = 1
                continue
            err2, result2 = parent.utm.add_nlist_items(named_list_id, tmp_content)
            if err2:
        ...
        if item['name'] not in ['Parental Control', 'Productivity', 'Safe categories', 'Threats',
            ...
            for category in content:
                ...
                if err2 == 1:
                    ...
                    parent.stepChanged.emit(f'2|   Содержимое категории "{category["name"]}" не добавлено, так как уже существует.')
                else:
...
def import_application_groups(parent):
    ...
    for item in data:
        ...
        for app in content:
            try:
                new_app.append({'value': l7_app_id[app['value']]})
            except KeyError as err:
                parent.stepChanged.emit(f'4|   Ошибка! Приложение "{app_name}" не импортировано. Такого приложения нет на UG NGFW.')
                error = 1
                continue
        err2, result2 = parent.utm.add_nlist_items(list_applicationgroups[item['name']], new_app)
        if err2:
            parent.stepChanged.emit(f'{err2}|   {result2}')
            if err2 == 1:
                error = 1
        else:
            parent.stepChanged.emit(f'2|   Содержимое группы приложений "{item["name"]}" добавлено.')
...
#    servicegroups_list = {x['name']: x['id'] for x in result}
    ...
    for item in data:
        ...
        for service in item['services']:
            try:
                new_services.append(services_list[service[1]])
            except KeyError as err:
                error = 1
