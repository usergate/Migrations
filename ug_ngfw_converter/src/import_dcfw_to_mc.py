...
class ImportMcDcfwSelectedPoints(QThread, ReadWriteBinFile, MyMixedService):
    ...
    def run(self):
        ...
        # Получаем список всех шаблонов DCFW области.
        if 'realm_templates' not in self.mc_data:
    ...
    def import_services_list(self, path):
        ...
        for item in data:
            ...
            if item['name'] in services:
                ...
                err, result = self.utm.add_dcfw_template_service(self.template_id, item)
                if err == 3:
        ...
        for item in data:
            ...
            if item['name'] in servicegroups:
                if self.template_id == servicegroups[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_nlist(self.template_id, servicegroups[item['name']].id, item)
                    if err == 1:
                ...
                err, result = self.utm.add_dcfw_template_nlist(self.template_id, item)
                if err == 1:
                ...
                if content:
                    for service in content:
                        ...
                        if tmp.template_id == self.template_id:
                            ...
                            error = 1
                            continue
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, servicegroups[item['name']].id, service)
                        if err2 == 3:
...
#                                self.stepChanged.emit(f'BLACK|       Добавлен {item_value}.')
    ...
    def import_url_lists(self, path):
        ...
        for file_name in files_list:
            ...
            if self.template_id == url_lists[data['name']].template_id:
                if data['list_type_update'] == 'static':
                    if data['content']:
                        err, result = self.utm.add_dcfw_template_nlist_items(self.template_id, url_lists[data['name']].id, data['content'])
                        if err == 1:
            ...
            if item['list_type_update'] == 'static':
                if content:
                    for value in content:
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, calendars[item['name']].id, value)
                        if err2 == 1:
                            error = 1
        ...
        for item in data:
            ...
            if item['name'] in shapers:
                if self.template_id == shapers[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_shaper(self.template_id, shapers[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_shaper(self.template_id, item)
                if err:
    ...
    def import_templates_list(self, path):
        ...
        default_template_names = {'blockpage_en', 'blockpage_en', }
        error = 0
        ...
        for item in data:
            if item['name'] in response_pages:
                if self.template_id == response_pages[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_responsepage(self.template_id, response_pages[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_responsepage(self.template_id, item)
                if err:
                    self.stepChanged.emit(f'RED|    {result}  [Шаблон страницы "{item["name"]}" не импортирован]')
                    error = 1
                    continue
            ...
            if f"{item['name']}.html" in html_files:
                ...
                if err:
                    ...
                    err2, result2 = self.utm.set_dcfw_template_responsepage_data(self.template_id, response_pages[item['name']].id, result['storage_file_uid'])
                    if err2:
            ...
            if item['list_type_update'] == 'static':
                if content:
                    for category in content:
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, url_category_groups[item['name']].id, category)
                        if err2 == 3:
        ...
        for item in data:
            ...
            if item['name'] in custom_url:
                if self.template_id == custom_url[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_custom_url(self.template_id, custom_url[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_custom_url(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in users_apps:
                if self.template_id == users_apps[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_app_signature(self.template_id, users_apps[item['name']].id, item)
                    if err:
                    ...
                    if users_apps[item['name']].template_id in self.templates:
                        ...
                        self.stepChanged.emit(f'RED|    Error: Пользовательское приложение "{item["name"]}" уже существует в шаблоне '
                                              f'"{self.mc_data["realm_templates"][users_apps[item["name"]].template_id]}" '
                                               'который отсутствует в данной группе шаблонов.\n       Пользовательское приложение не импортировано.')
                        error = 1
            else:
                err, result = self.utm.add_dcfw_template_app_signature(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in l7_profiles:
                if self.template_id == l7_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_l7_profile(self.template_id, l7_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_l7_profile(self.template_id, item)
                if err:
            ...
            if item['list_type_update'] == 'static':
                if content:
                    for app in content:
                        ...
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, apps_groups[item['name']].id, app) 
                        if err2 == 1:
            ...
            if item['list_type_update'] == 'static':
                if content:
                    for email in content:
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, email_groups[item['name']].id, email)
                        if err2 == 1:
            ...
            if item['list_type_update'] == 'static':
                if content:
                    for number in content:
                        err2, result2 = self.utm.add_dcfw_template_nlist_item(self.template_id, phone_groups[item['name']].id, number)
                        if err2 == 1:
        ...
        for item in data:
            if item['msg'] in users_signatures:
                if self.template_id == users_signatures[item['msg']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_idps_signature(self.template_id, users_signatures[item['msg']].id, item)
                    if err:
                    ...
                    if users_signatures[item['msg']].template_name:
                        ...
                        self.stepChanged.emit(f'RED|    Error: Сигнатура СОВ "{item["msg"]}" уже существует в шаблоне '
                                              f'"{self.mc_data["realm_templates"][users_signatures[item["msg"]].template_id]}" '
                                               'который отсутствует в данной группе шаблонов.\n       Пользовательское приложение не импортировано.')
                        error = 1
            else:
                err, result = self.utm.add_dcfw_template_idps_signature(self.template_id, item)
                if err:
    ...
    def import_idps_profiles(self, path):
        ...
        err, result = self.utm.get_dcfw_template_idps_signatures(self.template_id)
        if err:
        ...
        for item in data:
            if 'filters' not in item:
                self.stepChanged.emit('RED|    Импорт профилей СОВ старых версий не поддерживается для версий и выше.')
                error = 1
                break
            ...
            if item['name'] in idps_profiles:
                if self.template_id == idps_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_idps_profile(self.template_id, idps_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_idps_profile(self.template_id, item)
                if err:
                    error = 1
        ...
        for item in data:
            ...
            if item['name'] in notification_profiles:
                if self.template_id == notification_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_notification_profile(self.template_id, notification_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_notification_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in netflow_profiles:
                if self.template_id == netflow_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_netflow_profile(self.template_id, netflow_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_netflow_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in lldp_profiles:
                if self.template_id == lldp_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_lldp_profile(self.template_id, lldp_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_lldp_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in ssl_profiles:
                if self.template_id == ssl_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_ssl_profile(self.template_id, ssl_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_ssl_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in ssl_forward_profiles:
                if self.template_id == ssl_forward_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_ssl_forward_profile(self.template_id, ssl_forward_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_ssl_forward_profile(self.template_id, item)
                if err:
                    error = 1
        ...
        for item in data:
            if item['name'] in bfd_profiles:
                if self.template_id == bfd_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_bfd_profile(self.template_id, bfd_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_bfd_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            if item['name'] in userid_filters:
                if self.template_id == userid_filters[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_useridagent_filter(self.template_id, userid_filters[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_useridagent_filter(self.template_id, item)
                if err:
    ...
    def import_zones(self, path):
        ...
        for zone in data:
            ...
            err, result = self.utm.add_dcfw_template_zone(self.template_id, zone)
            if err == 3:
                self.stepChanged.emit(f'uGRAY|    {result}')
            elif err == 1:
...
#                else:
                ...
                if iface_name in mc_ifaces:
                    ...
                    continue
                if item['name'] == 'port0':
                    self.stepChanged.emit(f'bRED|       Интерфейс "{item["name"]}" не может быть импортирован в шаблон МС.')
                    continue
                ...
                err, result = self.utm.add_dcfw_template_interface(self.template_id, item)
                if err:
...
#                else:
        ...
        for item in data:
            ...
            if gateway_name in gateways:
                if self.template_id == gateways[gateway_name].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_gateway(self.template_id, gateways[gateway_name].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_gateway(self.template_id, item)
                if err:
    ...
    def import_gateway_failover(self, path):
        ...
        err, result = self.utm.update_dcfw_template_gateway_failover(self.template_id, data)
        if err:
...
#            else:
            ...
            err, result = self.utm.add_dcfw_template_dhcp_subnet(self.template_id, item)
            if err == 1:
    ...
    def import_dns_servers(self, path):
        ...
        for item in data:
            ...
            err, result = self.utm.add_dcfw_template_dns_server(self.template_id, item)
            if err == 3:
                self.stepChanged.emit(f'GRAY|    {result}')
            elif err == 1:
    ...
    def import_dns_proxy(self, path):
        ...
        for key, value in result.items():
            ...
            err, result = self.utm.update_dcfw_template_dns_setting(self.template_id, key, value)
            if err:
                self.stepChanged.emit(f'RED|    {result}')
                error = 1
        if error:
    ...
    def import_dns_rules(self, path):
        ...
        for item in data:
            ...
            err, result = self.utm.add_dcfw_template_dns_rule(self.template_id, item)
            if err == 3:
                self.stepChanged.emit(f'GRAY|    {result}')
            elif err == 1:
    ...
    def import_dns_static(self, path):
        ...
        for item in data:
            err, result = self.utm.add_dcfw_template_dns_static_record(self.template_id, item)
            if err == 3:
                self.stepChanged.emit(f'GRAY|    {result}')
            elif err == 1:
...
#            else:
            ...
            if item['ospf']:
                ...
                if item['ospf']['redistribute'] and isinstance(item['ospf']['redistribute'][0], str):
                    self.stepChanged.emit(f'RED|    Error: [VRF "{item["name"]}"] Route redistributtion удалёно из настроек OSPF так как структура не соответствует версии МС.')
            ...
            try:
                if vrf_name in mc_vrf:
                    ...
                    err, result = self.utm.update_dcfw_template_vrf(self.template_id, mc_vrf[vrf_name].id, item)
                    if err:
                        self.stepChanged.emit(f'RED|       {result}\n        [VRF "{item["name"]}"] не обновлён.')
                        error = 1
                    else:
                        self.stepChanged.emit(f'uGRAY|       VRF "{item["name"]}" обновлён.')
                else:
                    err, result = self.utm.add_dcfw_template_vrf(self.template_id, item)
                    if err:
                        self.stepChanged.emit(f'RED|    {result}\n       [VRF "{item["name"]}" не импортирован]')
                        error = 1
                    else:
                        mc_vrf[vrf_name] = BaseObject(id=result, template_id=self.template_id, template_name=self.templates[self.template_id])
        ...
        for cert_name, cert_path in certificates.items():
            ...
            if 'cert.pem' in files:
                ...
                if data['name'] in mc_certs:
                    ...
                    err, result = self.utm.new_dcfw_template_certificate(self.template_id, data)
                    if err == 1:
                        self.stepChanged.emit(f'RED|       {result}')
                        error = 1
                    elif err == 3:
            ...
            if data['name'] in mc_certs:
                if self.template_id == mc_certs[data['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_certificate(self.template_id, mc_certs[data['name']].id, data, cert_data, private_key=key_data)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_certificate(self.template_id, data, cert_data, private_key=key_data)
                if err:
    ...
    def import_client_certificate_profiles(self, path):
        ...
        for item in data:
            if item['name'] in client_certs_profiles:
                ...
                err, result = self.utm.add_dcfw_template_client_certificate_profile(self.template_id, item)
                if err == 1:
...
#        self.import_proxy_exceptions(path)
    ...
    def import_ui(self, path):
        ...
        for key in data:
            if key in params:
                ...
                err, result = self.utm.set_dcfw_template_general_settings(self.template_id, setting)
                if err:
    ...
    def import_proxy_port(self, path):
        ...
        err, result = self.utm.set_dcfw_template_general_settings(self.template_id, {'proxy_server_port': {'value': data}})
        if err:
    ...
    def import_cache_settings(self, path):
        ...
        for key in settings:
            err, result = self.utm.set_dcfw_template_general_settings(self.template_id, settings[key])
            if err:
    ...
    def import_local_groups(self, path):
        ...
        for item in groups:
            ...
            if item['name'] in local_groups:
                ...
                err, result = self.utm.add_dcfw_template_group(self.template_id, item)
                if err == 1:
                ...
                if len(user_array) > 1 and ('\\' in user_array[1]):
                    ...
                    try:
                        ...
                        if err1:
                            ...
                            continue
                        err2, result2 = self.utm.add_dcfw_user_in_group(self.template_id, local_groups[item['name']].id, result1)
                        if err2:
    ...
    def import_local_users(self, path):
        ...
        for item in users:
            ...
            if item['name'] in local_users:
                ...
                err, result = self.utm.add_dcfw_template_user(self.template_id, item)
                if err == 1:
            ...
            for group in user_groups:
                try:
                    ...
                    err2, result2 = self.utm.add_dcfw_user_in_group(self.template_id, group_guid, local_users[item['name']].id)
                    if err2:
    ...
    def import_ldap_servers(self, path, ldap_servers):
        ...
        for item in data:
            ...
            if item['name'] in ldap_servers:
                ...
                item['type'] = 'ldap'
                item.pop("cc", None)
                err, result = self.utm.add_dcfw_template_auth_server(self.template_id, item)
                if err:
    ...
    def import_2fa_profiles(self, path):
        ...
        for item in data:
            ...
            if item['name'] in profiles_2fa:
                ...
                err, result = self.utm.add_dcfw_template_2fa_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in auth_profiles:
                if self.template_id == auth_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_auth_profile(self.template_id, auth_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_auth_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in captive_profiles:
                if self.template_id == captive_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_captive_profile(self.template_id, captive_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_captive_profile(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in captive_portal_rules:
                if self.template_id == captive_portal_rules[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_captive_portal_rule(self.template_id, captive_portal_rules[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_captive_portal_rule(self.template_id, item)
                if err:
    ...
    def set_useridagent_config(self, data, useridagent_config):
        ...
        if self.node_name in useridagent_config:
            if self.template_id == useridagent_config[self.node_name].template_id:
                ...
                err, result = self.utm.update_dcfw_template_useridagent_config(self.template_id, useridagent_config[data['name']].id, data)
                if err:
            ...
            err, result = self.utm.add_dcfw_template_useridagent_config(self.template_id, data)
            if err:
    ...
    def import_agent_servers(self, path):
        ...
        for item in data:
            ...
            if srv_name in useridagent_servers:
                ...
                err, result = self.utm.add_dcfw_template_useridagent_server(self.template_id, item)
                if err:
...
#-------------------------------------- Политики сети ---------------------------------------------------------
    def import_firewall_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_firewall_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in firewall_rules:
                ...
                item['position'] = 'last'
                err, result = self.utm.add_dcfw_template_firewall_rule(self.template_id, item)
                if err:
                    error = 1
    ...
    def import_nat_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_traffic_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in nat_rules:
                ...
                item.pop('position', None)
                err, result = self.utm.update_dcfw_template_traffic_rule(self.template_id, nat_rules[item['name']], item)
                if err:
                    error = 1
                ...
                item['position'] = 'last'
                err, result = self.utm.add_dcfw_template_traffic_rule(self.template_id, item)
                if err:
                    error = 1
    ...
    def import_loadbalancing_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_loadbalancing_rules(self.template_id)
        if err:
    ...
    def import_loadbalancing_tcpudp(self, path, balansing_servers):
        ...
        for item in data:
            ...
            if item['name'] in tcpudp_rules:
                ...
                err, result = self.utm.update_dcfw_template_loadbalancing_rule(self.template_id, tcpudp_rules[item['name']], item)
                if err:
                    self.stepChanged.emit(f'RED|          {result}  [Правило "{item["name"]}"]')
                    error = 1
                else:
                ...
                err, result = self.utm.add_dcfw_template_loadbalancing_rule(self.template_id, item)
                if err:
                    self.stepChanged.emit(f'RED|       {result}  [Правило "{item["name"]}" не импортировано]')
                    error = 1
                else:
    ...
    def import_shaper_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_shaper_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in shaper_rules:
                ...
                item.pop('position', None)
                err, result = self.utm.update_dcfw_template_shaper_rule(self.template_id, shaper_rules[item['name']], item)
                if err:
                    error = 1
                    self.stepChanged.emit(f'RED|       {result}  [Правило "{item["name"]}"]')
                else:
                ...
                item['position'] = 'last'
                err, result = self.utm.add_dcfw_template_shaper_rule(self.template_id, item)
                if err:
                    error = 1
                    self.stepChanged.emit(f'RED|    {result}  [Правило "{item["name"]}" не импортировано]')
                else:
        ...
        for item in data:
            ...
            if item['name'] in security_profiles:
                if self.template_id == security_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_vpn_client_security_profile(self.template_id, security_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_vpn_client_security_profile(self.template_id, item)
                if err:
    ...
    def import_vpnserver_security_profiles(self, path):
        ...
        for item in data:
            ...
            if item['name'] in security_profiles:
                if self.template_id == security_profiles[item['name']].template_id:
                    self.stepChanged.emit(f'uGRAY|    Профиль безопасности VPN "{item["name"]}" уже существует в текущем шаблоне.')
                    err, result = self.utm.update_dcfw_template_vpn_server_security_profile(self.template_id, security_profiles[item['name']].id, item)
                    if err:
                        self.stepChanged.emit(f'RED|       {result}  [Профиль безопасности VPN "{item["name"]}"]')
                        error = 1
                    else:
                ...
                err, result = self.utm.add_dcfw_template_vpn_server_security_profile(self.template_id, item)
                if err:
                    self.stepChanged.emit(f'RED|    {result}  [Профиль безопасности VPN "{item["name"]}" не импортирован]')
                    error = 1
                else:
        ...
        for item in data:
            ...
            if item['name'] in vpn_networks:
                if self.template_id == vpn_networks[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_vpn_network(self.template_id, vpn_networks[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_vpn_network(self.template_id, item)
                if err:
    ...
    def import_vpn_client_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_vpn_client_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in vpn_client_rules:
                ...
                err, result = self.utm.update_dcfw_template_vpn_client_rule(self.template_id, vpn_client_rules[item['name']], item)
                if err:
                ...
                err, result = self.utm.add_dcfw_template_vpn_client_rule(self.template_id, item)
                if err:
    ...
    def import_vpn_server_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_vpn_server_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in vpn_server_rules:
                ...
                err, result = self.utm.update_dcfw_template_vpn_server_rule(self.template_id, vpn_server_rules[item['name']], item)
                if err:
                ...
                item['position'] = 'last'
                err, result = self.utm.add_dcfw_template_vpn_server_rule(self.template_id, item)
                if err:
    ...
    def import_notification_alert_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_notification_alert_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in alert_rules:
                ...
                err, result = self.utm.update_dcfw_template_notification_alert_rule(self.template_id, alert_rules[item['name']], item)
                if err:
                ...
                err, result = self.utm.add_dcfw_template_notification_alert_rule(self.template_id, item)
                if err:
        ...
        for item in data:
            ...
            if item['name'] in snmp_security_profiles:
                if self.template_id == snmp_security_profiles[item['name']].template_id:
                    ...
                    err, result = self.utm.update_dcfw_template_snmp_security_profile(self.template_id, snmp_security_profiles[item['name']].id, item)
                    if err:
                ...
                err, result = self.utm.add_dcfw_template_snmp_security_profile(self.template_id, item)
                if err:
    ...
    def import_snmp_settings(self, path):
        ...
        err, result = self.utm.add_dcfw_template_snmp_parameters(self.template_id, data)
        if err == 1:
            self.error = 1
    ...
    def import_snmp_rules(self, path):
        ...
        err, result = self.utm.get_dcfw_template_snmp_rules(self.template_id)
        if err:
        ...
        for item in data:
            ...
            if item['name'] in snmp_rules:
                ...
                err, result = self.utm.update_dcfw_template_snmp_rule(self.template_id, snmp_rules[item['name']], item)
                if err:
                ...
                err, result = self.utm.add_dcfw_template_snmp_rule(self.template_id, item)
                if err:
    ...
    def add_empty_vrf(self, vrf_name, ports):
        ...
        err, result = self.utm.add_dcfw_template_vrf(self.template_id, vrf)
        if err:
            return err, result
    ...
    def add_new_nlist(self, name, nlist_type, content):
        ...
        err, list_id = self.utm.add_dcfw_template_nlist(self.template_id, nlist)
        if err:
            return err, list_id
        err, result = self.utm.add_dcfw_template_nlist_items(self.template_id, list_id, content)
        if err:
            return err, result
        return 0, list_id
