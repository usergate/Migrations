...
Данное поведение будет исправлено в версии МС 7.1.3.<br>
2. В списках морфологии слова добавляются не корректно. К слову добавляется его вес через символ табуляции. Это не ошибка
конвертера, а баг МС. Возможно будет исправлен в версии МС 7.1.2.<br>
3. Перед импортом каждого объекта происходит поиск по его имени во всех шаблонах указанной группы шаблонов. Если такой
объект найден в каком то из шаблонов, то он не импортируется. При этом выдаётся соответствующее сообщение с указанием
шаблона в котором объект присутствует. Шаблоны их других групп не проверяются (данное действие находится в разработке и
пока работет не со всеми объектами).
...
5. Перед импортом убедитесь, что в шаблоне МС существуют необходимые интерфейсы и зоны. Они необходимы для создания VLAN и
...
затем создать необходимые интерфейсы для будущих VLAN и subnet DHCP и только затем импортировать всё.<br>
6. При импорте сертификатов, если в каталоге сертификата присутствует приватный ключ (key.pem или key.der), то он
импортируется вместе с сертификатом. Если в каталоге нет файлов cert.pem и cert.der, то генерируется новый сетрификат вместе
с приватным ключём на основе информации файла certificate_list.<br>
7. После импорта раздела UserGate/Настройки, включите синхронизацию необходимых подразделов.<br>
8. Если при импорте правил в шаблон МС произошли ошибки, то данное правило становиться не активным и в описание правила
...
9. Правила МЭ, КФ и т.д. добавляются в конец списка правил. Это надо учитывать если вы повторно импортируете конфигурацию
в шаблон после изменений в файлах json. Не меняйте имена правил, иначе правила продублируются в конец списка правил.<br>
10. НЕ импортируются: "Библиотеки/Syslog фильтры агента UserID" - так как пока не работает соответсвующий API, Серверные и
Клиентские профили безопасности VPN - если вы импортируете конфигурацию с версий 5 и 6 так как в этих версиях таких
профилей безопасности нет.
...
получим ошибку.<br>
3. Настройки VRF в кластерной конфигурации не импортируются.<br>
4. Для версии 5:<br>
...
11. Сертификаты экспортируются, но импорт возможен только в шаблон МС. На NGFW пока импорт не реализован. Если используется сертификат по умолчанию для SSL инспектирования
...
14. При импорте, правила МЭ, КФ и т.д. добавляются в конец списка правил. Если правило уже существует, то оно обновляется и его
...
15. Если при импорте правил произошли ошибки, то данное правило становиться не активным и в описание правила добавляются
