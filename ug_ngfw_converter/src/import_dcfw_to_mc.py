...
class ImportMcDcfwSelectedPoints(QThread, ReadWriteBinFile, MyMixedService):
    ...
    def import_bond_interfaces(self, path, data):
        """Импортируем Бонд и Bridge интерфесы."""
            ...
            if data['name'] in mc_certs:
                ...
                if not cert_data:
                    ...
                    if data['role'] in self.mc_data['cert_roles']:
                        ...
                        err, result = self.utm.update_dcfw_template_certificate(self.template_id, mc_certs[data['name']].id, {'role': data['role']})
                        if err:
